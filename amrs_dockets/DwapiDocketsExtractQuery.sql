...
FROM
    ndwr.ndwr_patient_art_extract a
        JOIN
    ndwr.ndwr_selected_site s USING (SiteCode)
        JOIN
    ndwr.ndwr_all_patients_extract e ON (e.PatientPK = a.PatientPK
        AND a.SiteCode = e.SiteCode)
WHERE
    a.LastVisit >= a.StartARTDate
...
SELECT 
    p.PatientPK,
    CASE
      WHEN p.PatientID IS NULL THEN p.PatientPK
      ELSE p.PatientID
    END AS 'PatientID',
    p.FacilityID,
    p.SiteCode,
    ...
    p.DispenseDate,
    p.Duration,
    IF(p.ExpectedReturn IS NULL
            || DATE(p.ExpectedReturn) = '0000-00-00',
        DATE_ADD(p.DispenseDate, INTERVAL 21 DAY),
        p.ExpectedReturn) AS ExpectedReturn,
    p.TreatmentType,
    p.RegimenLine,
    p.PeriodTaken,
    p.ProphylaxisType,
    p.RegimenChangedSwitched,
    p.RegimenChangeSwitchReason,
    p.StopRegimenReason,
    p.StopRegimenDate
FROM
    ndwr.ndwr_pharmacy p
        JOIN
    ndwr.ndwr_selected_site USING (SiteCode)
    join ndwr.ndwr_all_patients_extract e on (e.PatientPK = p.PatientPK AND e.SiteCode = p.SiteCode)
WHERE
    DATE(p.DispenseDate) >= '1997-01-01'
    group by p.VisitID;
