CREATE PROCEDURE `buildNDWR_2_QueueData`(IN selectedMFLCode INT)
BEGIN
  DECLARE n INT DEFAULT 0;
         ...
         delete from ndwr_baseline_queue_2 where person_id in(select patientid from ndwr.ndwr_base_line);
		 SELECT count(*) from ndwr_baseline_queue_2  INTO n;
         ...
         WHILE i<n DO  		
				...
				call createPatientNDWRDataSets(selectedMFLCode,selectedFacility,selectedPatient);	
 				insert into progress(status,queue) select  concat(((i/n) * 20),' % of ', n) as '% complete', 2 as queue;
				           	   
           SET i = i + 1;
  end while; 
  delete from ndwr_baseline_queue_2;             
