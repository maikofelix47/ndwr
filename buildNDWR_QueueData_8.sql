CREATE  PROCEDURE `buildNDWR_QueueData_8`()
BEGIN
 DECLARE i INT Default 0;
 DECLARE n INT Default 0;
 DECLARE selectedMFLCode INT Default 0;
  		 DECLARE selectedPatient INT Default 0;
  		 DECLARE selectedFacility varchar(800) Default 0;        
          ...
          delete from ndwr.ndwr_baseline_queue_8 where person_id in(select patientid from ndwr.ndwr_base_line);
 		 SELECT count(*) from ndwr.ndwr_baseline_queue_8  INTO n;
...
SET i=0;
          ...
          WHILE i<n DO  		
 				SELECT distinct person_id  FROM ndwr_baseline_queue_8 limit i,1 into selectedPatient; 
 				
 				call createPatientNDWRDataSets(selectedMFLCode,selectedFacility,selectedPatient);	
  				insert into progress(status,queue) select  concat(((i/n) * 100),' % of ', n) as '% complete', 8 as queue;
