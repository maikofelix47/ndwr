CREATE  PROCEDURE `generate_prep_summary_v1_3_prod`(IN query_type varchar(50), IN queue_number int, IN queue_size int, IN cycle_size int , IN log boolean)
BEGIN
					...
					CREATE TABLE IF NOT EXISTS `ndwr.flat_prep_summary_v1_1` (
                    ...
                    CREATE TABLE IF NOT EXISTS `ndwr.prep_weekly_report_dataset_v1_1` (
                    ...
                    if(@query_type="build") then
							...
							SET @dyn_sql=CONCAT('Create table if not exists ',@queue_table,'(person_id int primary key) (select patient_id as person_id from flat_prep_summary_build_queue limit ', queue_size, ');'); 
                            ...
                            SET @dyn_sql=CONCAT('delete t1 from flat_prep_summary_build_queue t1 join ',@queue_table, ' t2 using (person_id)'); 
							PREPARE s1 from @dyn_sql; 
							EXECUTE s1; 
							DEALLOCATE PREPARE s1; 
...
--                             DEALLOCATE PREPARE s1;  
                    ...
                    if (@query_type="sync") then
							...
							select "Finding patients in person's table...";  
                            replace into flat_prep_summary_sync_queue
...
-- 							end as prep_start_date_branch,
						...
						(SELECT
						   concat(week, person_id) as elastic_id,
							...
							null as turned_positive_this_week,
