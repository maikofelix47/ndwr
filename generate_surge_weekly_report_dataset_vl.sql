CREATE  PROCEDURE `generate_surge_weekly_report_dataset_vl`(IN query_type varchar(50), IN queue_number int, IN queue_size int, IN cycle_size int , IN log boolean)
BEGIN
  			set @start = now();
  			set @table_version = "surge_weekly_report_dataset_01";
  			set @last_date_created = (select max(date_created) from flat_hiv_summary_v15b);
            set @may_2019 = '2019-05-12';
			set @may_2015 = '2019-05-12';
 			set @october_2018 = '2018-10-01';
             
CREATE TABLE IF NOT EXISTS surge_weekly_report_dataset_t0 (
    ...
    old_ltfus_to_dead_this_week INT(0),
    old_ltfus_to_transfer_out_this_week INT(0),
    dc_eligible_this_week INT(0),
    dc_eligible_and_scheduled_this_week INT(0),
    active_on_dc INT(0),
    ever_active_after_october_18 INT(0),
    ...
    old_ltfus_to_dead INT(0),
    old_ltfus_to_transfer_out INT(0),
...
SELECT CONCAT('creating patient_vl ...');
 ...
 CREATE Temporary TABLE if not exists patient_vl_status (
   ...
   patients_due_for_vl int(1) DEFAULT NULL,
...
SELECT CONCAT('creating surge_weekly_report_dataset_0 ...');    
  ...
  FROM
      surge_week t1
         ...
         left join etl.surge_base_data using(person_id)
         left join ndwr.patient_breast_feeding bf using(encounter_id)
      JOIN
...
SELECT CONCAT('creating surge_weekly_report_dataset__1 ...');
 ...
 CREATE temporary TABLE surge_weekly_report_dataset_1 (SELECT sw0.*,
   ...
   IF( death_date>=@may_2019, 1,0) as cumulative_dead,
    CASE
          WHEN YEARWEEK(vl.end_date) >= YEARWEEK(death_date) THEN @status:='dead'
          WHEN YEARWEEK(vl.end_date) >= YEARWEEK(transfer_out_date) THEN @status:='transfer_out'
          WHEN  days_since_rtc_date > 0  AND days_since_rtc_date < 5 
           AND death_date IS NULL AND transfer_out_date IS NULL THEN  @status:='missed'
          WHEN  days_since_rtc_date > 4       and days_since_rtc_date <= 28 
          	AND death_date IS NULL AND transfer_out_date IS NULL THEN  @status:='defaulter'
          WHEN  days_since_rtc_date <= 28  THEN  @status:='active'            
          WHEN  days_since_rtc_date > 28  THEN  @status:='ltfu'         
          ELSE @status:='unknown'
      END AS status,
      IF(scheduled_this_week =1 and on_schedule_this_week=0 and  days_since_rtc_date >=0  
      and days_since_rtc_date <5 and @status not in('transfer_out','dead'), 1, 0) AS missed_appointment_this_week ,
      
      IF(days_since_rtc_date >=0  and days_since_rtc_date <5 and @status not in('transfer_out','dead'), 1, 0) AS missed_cumulative ,
      
       CASE
          WHEN
                 days_since_rtc_date > 4 and days_since_rtc_date <=28 and @status not in('transfer_out','dead') 
      ...
      CASE
          WHEN
                 days_since_rtc_date > 28 and @status not in('transfer_out','dead') 
      ...
      CASE
          WHEN
                 days_since_rtc_date > 0 and days_since_rtc_date <5 and @status not in('transfer_out','dead') 
     ...
     t6.weight AS weight,
     t6.height AS height,
     fa.encounter_datetime as death_reporting_date,
     t6.bmi FROM
     surge_weekly_report_dataset_0 sw0
     ...
     surge_vitals t6 ON t6.encounter_id = sw0.encounter_id
    	LEFT JOIN etl.flat_appointment fa ON (sw0.person_id = fa.person_id and fa.encounter_type = 31));
...
SELECT CONCAT('creating surge_weekly_report_dataset__2 ...');
 ...
 CREATE temporary TABLE surge_weekly_report_dataset__2 (SELECT *,
     ...
     @ltfu_surge_base:=ltfu_surge_base AS cur_ltfu_surge_base,
     CASE
         WHEN
             @prev_id = @cur_id
     ...
     CASE
         WHEN
             (@prev_id != @cur_id
                 ...
                 AND year_week >= 201835
                 AND year_week < 201918
                 AND (@cur_status = 'active' OR @cur_status:='defaulter')
         THEN
     ...
     CASE
         WHEN
             (@prev_id != @cur_id
                 ...
                 AND year_week <= 201918
                 AND ( death_date IS NULL OR (death_date IS NOT NULL and DATE(death_reporting_date) > '2019-05-11') )
                 AND ( transfer_out_date IS NULL OR (transfer_out_date IS NOT NULL and DATE(encounter_datetime) > '2019-05-11') )
	...
	CASE
	     WHEN @prev_id = @cur_id then @baseline_location
	     WHEN @is_ltfu_surge_baseline = 1 THEN @baseline_location:= location_id
	     ELSE @baseline_location:= null
	END as  baseline_location,
    ...
    CASE
     ...
     CASE
         ...
         THEN
             'transfer_out'
     END AS ltfu_outcome_this_week,
     CASE
         WHEN
             @prev_id = @cur_id
                 AND @is_ltfu_surge_baseline
                 AND @prev_status = 'ltfu'
                 AND @cur_status = 'active'
         THEN
             'returned_to_care'
         ...
         THEN
             'transfer_out'
     END AS surge_baseline_ltfu_outcome_this_week,
     CASE
         WHEN @prev_id != @cur_id THEN @height:=height
         WHEN
     ...
     IF(ltfu_surge_base = 1, 1, 0) AS ltfu_surge_baseline,
     ...
     IF((@is_ltfu_surge_baseline = 1 AND @cur_status = 'ltfu') OR (@is_ltfu_after_may_revised = 1 AND @cur_status = 'ltfu'  AND death_date IS NULL AND transfer_out_date IS NULL), 1, 0) AS surge_ltfu_and_ltfu_after_may,
    ...
    IF(((@is_ltfu_surge_baseline = 1 AND @prev_status = 'ltfu') OR (@is_ltfu_after_may_revised = 1 AND @prev_status = 'ltfu'))
     ...
     IF(@was_ltfu_before_october_2018 = 1
         AND @cur_status = 'dead',
     1,
     0) AS old_ltfus_to_dead,
         
     IF(@was_ltfu_before_october_2018 = 1
             AND @cur_status = 'transfer_out',
         1,
         0) AS old_ltfus_to_transfer_out,
     ...
     IF(@was_ltfu_before_october_2018 = 1
         ...
         AND @cur_status = 'dead',
     1,
     0) AS old_ltfus_to_dead_this_week,
         
     IF(@was_ltfu_before_october_2018 = 1
     	AND @prev_status = 'ltfu'
             AND @cur_status = 'transfer_out',
         1,
         0) AS old_ltfus_to_transfer_out_this_week,
     ...
     # if all patients on DC are elligible for DC, these indicators eligible_and_on_dc  and  on_dc_this_week should be equal
     
     # since the two are not equal, does it mean some patients are on DC yet they have not qualified to be there
     
     # however if all patients on DC are elligible for DC, that means a quick fix to the problem we are having indicator elligible_total   should be
...
SELECT 
    COUNT(*) patients_for_24
FROM
    surge_weekly_report_dataset__2
WHERE
    year_week = 201924;
       
SELECT CONCAT('replacing into surge_weekly_report_dataset ...');
  replace into surge_weekly_report_dataset_t0								  
  			(select
					 now(),
					 elastic_id ,
					 person_uuid ,
					 ...
					 missed_this_week ,
					 all_ltfus ,
					 ltfu_surge_base as ltfu_surge_baseline ,
					 ...
					 cumulative_dead ,
					 old_ltfus_to_dead_this_week,
					 old_ltfus_to_transfer_out_this_week,
					 dc_eligible_this_week,
					 dc_eligible_and_scheduled_this_week,
					 active_on_dc,
					 ever_active_after_october_18,
