CREATE PROCEDURE `buildNDWR_QueueData_11`()
BEGIN
 DECLARE i INT Default 0;
 DECLARE n INT Default 0;
 DECLARE selectedMFLCode INT Default 0;
  		 DECLARE selectedPatient INT Default 0;
  		 DECLARE selectedFacility varchar(1100) Default 0;        
          ...
          delete from ndwr.ndwr_baseline_queue_11 where person_id in(select patientid from ndwr.ndwr_base_line);
 		 SELECT count(*) from ndwr.ndwr_baseline_queue_11  INTO n;
...
SET i=0;
          ...
          WHILE i<n DO  		
 				SELECT distinct person_id  FROM ndwr_baseline_queue_11 limit i,1 into selectedPatient; 
 				
 				call createPatientNDWRDataSets(selectedMFLCode,selectedFacility,selectedPatient);	
  				insert into progress(status,queue) select  concat(((i/n) * 110),' % of ', n) as '% complete', 11 as queue;
