CREATE  PROCEDURE `generate_surge_weekly_report_dataset_v2`(IN query_type varchar(50), IN queue_number int, IN queue_size int, IN cycle_size int , IN log boolean)
BEGIN
  			set @start = now();
...
CREATE TABLE IF NOT EXISTS ndwr.surge_weekly_report_dataset_tak (
...
SELECT CONCAT('creating surge_weekly_report_dataset_0 ...');    
  ...
  (SELECT
    ...
    CASE
         WHEN
             visit_type NOT IN (24)
             AND med_pickup_rtc_date IS NOT NULL
         THEN
  ...
  FROM
      surge_week t1
          ...
          left join ndwr.patient_breast_feeding bf using(person_id) 
      JOIN
  ...
  WHERE
     t2.encounter_datetime < DATE_ADD(t1.end_date, INTERVAL 1 DAY)
          ...
          AND breast_feeding_encounter_date < DATE_ADD(t1.end_date, INTERVAL 1 DAY)
   ...
   IF( death_date>=@may_2019, 1,0) as cumulative_dead,
    CASE
           WHEN death_date IS NOT NULL AND YEARWEEK(sw0.end_date) >= YEARWEEK(death_date)  THEN @status:='dead'
               WHEN YEARWEEK(sw0.end_date) >= YEARWEEK(transfer_out_date) THEN @status:='transfer_out'
               
               WHEN  (days_since_rtc_date > 0  AND days_since_rtc_date < 5)
               		OR (days_since_med_pickup_rtc_date > 0  AND days_since_med_pickup_rtc_date < 5) THEN  @status:='missed'  
               
                WHEN  (days_since_rtc_date > 4  AND days_since_rtc_date <= 28) 
                	OR (days_since_med_pickup_rtc_date > 4  AND days_since_med_pickup_rtc_date < 28) THEN  @status:='defaulter'
              
              WHEN  days_since_med_pickup_rtc_date IS NOT NULL
              		AND days_since_rtc_date <= 0 AND days_since_med_pickup_rtc_date <= 0 
              		THEN  @status:='active' 
              		
              		WHEN days_since_med_pickup_rtc_date IS NULL
              		AND days_since_rtc_date <= 0 
              		THEN  @status:='active' 
              
             WHEN  days_since_rtc_date > 28 OR days_since_med_pickup_rtc_date > 28 THEN  @status:='ltfu'         
             ELSE @status:='unknown'
      END AS status,
      ...
      CASE
          WHEN
                 days_since_rtc_date > 28 OR days_since_med_pickup_rtc_date > 28 and @status not in('transfer_out','dead') 
 ...
 CREATE temporary TABLE surge_weekly_report_dataset__2 (SELECT *,
    ...
    CASE
     ...
     CASE
	     ...
	     THEN @week_patient_became_active:= year_week
	...
	END as  week_patient_became_active,
     ...
     IF(@is_ltfu_surge_baseline = 1 AND @cur_status = 'ltfu' 
      ...
      IF(@is_ltfu_after_may_revised = 1,
         1,
    ...
    IF(((@is_ltfu_surge_baseline = 1 AND @prev_status = 'ltfu') OR (@is_ltfu_after_may_revised = 1 AND @prev_status = 'ltfu'))
     ...
     IF(((@is_ltfu_surge_baseline = 1 AND @prev_status = 'ltfu') OR (@is_ltfu_after_may_revised = 1 AND @prev_status = 'ltfu')
     		OR ((days_since_rtc_date - 7 <= 29) AND days_since_rtc_date-6 > 28))
             AND (@cur_status = 'active'
             OR @cur_status = 'missed'
             OR @cur_status = 'defaulter'
             OR @cur_status = 'transfer_in'),
...
SELECT CONCAT('replacing into surge_weekly_report_dataset ...');
  replace into ndwr.surge_weekly_report_dataset_tak								  
  			(select
					 now(),
					 elastic_id ,
					 person_uuid ,
